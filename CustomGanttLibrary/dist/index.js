...
const TaskListHeaderDefault = _ref => {
  ...
  }, "\u00A0From"), React__default.createElement("div", {
    className: styles.ganttTable_HeaderSeparator,
    style: {
      height: headerHeight * 0.5,
...
const Gantt = _ref => {
  ...
  const [dateSetup, setDateSetup] = React.useState(() => {
  ...
  const [currentViewDate, setCurrentViewDate] = React.useState(undefined);
  const [taskListWidth, setTaskListWidth] = React.useState(0);
  const [svgContainerWidth, setSvgContainerWidth] = React.useState(0);
  const [svgContainerHeight, setSvgContainerHeight] = React.useState(ganttHeight);
  const [barTasks, setBarTasks] = React.useState([]);
  const [ganttEvent, setGanttEvent] = React.useState({
    action: ""
  });
  const taskHeight = React.useMemo(() => rowHeight * barFill / 100, [rowHeight, barFill]);
  const [selectedTask, setSelectedTask] = React.useState();
  const [failedTask, setFailedTask] = React.useState(null);
  const svgWidth = dateSetup.dates.length * columnWidth;
  const ganttFullHeight = barTasks.length * rowHeight;
  const [scrollY, setScrollY] = React.useState(0);
  const [scrollX, setScrollX] = React.useState(-1);
